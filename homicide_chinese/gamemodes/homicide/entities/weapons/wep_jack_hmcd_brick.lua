...
elseif(CLIENT)then
	...
	killicon.AddFont("wep_jack_hmcd_brick", "HL2MPTypeDeath", "5", Color(0, 0, 255, 255))
...
SWEP.ViewModel = "models/weapons/v_brick_c_arms.mdl"
...
function SWEP:PrimaryAttack()
	...
	if not(IsFirstTimePredicted())then
		timer.Simple(.1,function() if(IsValid(self))then self:DoBFSAnimation("throw") end end)
		return
	end
...
function SWEP:Deploy()
	...
	if(SERVER)then sound.Play("Concrete.ImpactSoft",self:GetPos(),65,math.random(90,110)) end
	return true
end
...
if(CLIENT)then
	...
	function SWEP:DrawWorldModel()
		...
		if(self.DatWorldModel)then
			...
			self.DatWorldModel=ClientsideModel("models/weapons/w_brick.mdl")
