...
function GM:DisplayEndRoundBoard(data)
	...
	if(data.reason==4)then
		if(self.ZOMBIE)then
			winner:SetText("幸存者们赢了! 阿尔法僵尸放弃了")
		elseif(self.SHTF)then
			winner:SetText("无辜者赢了! 叛徒放弃了")
		else
			winner:SetText("平民赢了! 凶手放弃了")
		end
		winner:SetTextColor(Color(255, 255, 255))
	elseif data.reason == 3 then
		if(self.ZOMBIE)then
			winner:SetText("幸存者们赢了! 阿尔法僵尸地退出了")
		elseif(self.SHTF)then
			winner:SetText("无辜者赢了! 叛徒愤怒地退出了")
		else
	...
	elseif data.reason == 2 then
		if(self.ZOMBIE)then
			winner:SetText("幸存者胜利")
		elseif(self.SHTF)then
			winner:SetText("无辜者胜利！")
		else
