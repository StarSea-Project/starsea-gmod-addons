...
function GM:PlayerSpawn( ply )
	...
	if(DatTeam==1)then
		...
		if(ply.RoundsSpectator>=3)then
			if not(ply:IsListenServerHost())then ply:Kick("旁观太久") else ply:Kill() end
			return
		end
	else
		...
		timer.Simple(i*2,function()
			if((IsValid(ply))and(ply:GetPhysicsObject()))then
				if(ply:GetPhysicsObject():IsPenetrating())then
					print(tostring(ply).." 出生时卡在某物中。重新定位")
					local spawnPoint=self:PlayerSelectTeamSpawn(ply:Team(),ply)
					if(IsValid(spawnPoint))then ply:SetPos(spawnPoint:GetPos());ply.HMCD_SpawnPos=spawnPoint:GetPos() end
				end
			end
	...
	timer.Simple(10,function()
		if((ply)and(IsValid(ply))and(ply:Alive())and(ply.Murderer))then
			...
			if(self.SHTF)then
				ply:PrintMessage(HUD_PRINTTALK,"国民警卫队将在 "..tostring(mins).." 分钟后到达。")
			else
				ply:PrintMessage(HUD_PRINTTALK,"警察将在 "..tostring(mins).." 分钟后到达。")
			end
		end
	end)
	...
	if !ply.Murderer then
		...
		if IsValid(attacker) && attacker:IsPlayer() then
			...
			elseif attacker != ply then
				...
				if(ply.Innocent)then
					if(self.SHTF)then
						msgs = Translator:AdvVarTranslate("{player} 杀死了一个无辜者。", {
					...
					msgs = Translator:AdvVarTranslate("{player} 杀死了一个咄咄逼人的捣蛋鬼。", {
						player = {text = attacker:GetBystanderName(), color = Color(col.x * 255, col.y * 255, col.z * 255)}
					})
				end
				if(self.DEATHMATCH)then
					msgs = Translator:AdvVarTranslate("{player} 杀了 {ded}", {
						player = {text = attacker:GetBystanderName(), color = Color(col.x * 255, col.y * 255, col.z * 255)},
		...
		if attacker != ply && IsValid(attacker) && attacker:IsPlayer() then
			...
			if(RealName==GameName)then
				if(self.ZOMBIE)then
					msgs=Translator:AdvVarTranslate("{player} 杀了阿尔法僵尸", {
						player = {text = GameName, color = Color(col.x * 255, col.y * 255, col.z * 255)}
					})
				elseif(self.SHTF)then
					msgs=Translator:AdvVarTranslate("{player} 杀了叛徒", {
						player = {text = GameName, color = Color(col.x * 255, col.y * 255, col.z * 255)}
					})
				else
...
function GM:PlayerSay(ply,text,teem)
	if ply:Team()==2 && ply:Alive() && self:GetRound()!=0 then
		...
		elseif(string.lower(text)=="*unstick")then
			...
			if(ply.Unstickable)then
				...
				if(((Wall)or(Playa)or(Still))and not(ply:InVehicle())and not(ply.HiddenInContainer))then
					...
					ply:PrintMessage(HUD_PRINTTALK,"玩家被卡住了。移动中...")
					...
					ply:PrintMessage(HUD_PRINTTALK,"无法确认被卡住的玩家。")
				end
			else
				ply:PrintMessage(HUD_PRINTTALK,"这一轮你已经用了*unstick。")
			end
		...
		local Extra=""
		if(WalkieTalkie)then Extra=" (无线电)" end
...
function GM:PlayerUse(ply,ent)
	...
	if(ent.ContactPoisoned)then
		if(ply.Murderer)then
			ply:PrintMessage(HUD_PRINTTALK,"这是被下了毒的!")
			return false
		else
...
function PlayerMeta:EnterContainer(ent)
	if((ent.PlayerHiddenInside)and(IsValid(ent.PlayerHiddenInside)))then
		self:PrintMessage(HUD_PRINTCENTER,"这个容器里已经有人了")
	else
...
function PlayerMeta:AntiCheat()
	...
	if(Weps)then
		for key,wep in pairs(Weps)do
			if not(wep.HomicideSWEP)then
				self:PrintMessage(HUD_PRINTTALK,wep:GetClass().." 不允许在sv_cheats 0下使用!")
	...
	if(self:GetMoveType()==MOVETYPE_NOCLIP)then
		...
		self:PrintMessage(HUD_PRINTTALK,"飞行不允许在sv_cheats 0下使用!")
	end
	if(self:HasGodMode())then
		self:GodDisable()
		self:PrintMessage(HUD_PRINTTALK,"无敌不允许在sv_cheats 0下使用!")
	end
	local Health=self:Health()
	if((Health)and(Health>105))then
		self:SetHealth(100)
		self:PrintMessage(HUD_PRINTTALK,"不允许在sv_cheats 0下加血!")
	end
	local Armor=self:Armor()
	if((Armor)and(Armor>0))then
		self:SetArmor(0)
		self:PrintMessage(HUD_PRINTTALK,"HL2 护甲不允许在sv_cheats 0下使用!")
	end
	...
	if(math.random(1,Chance)==1)then
		...
		elseif(DeathType==4)then
			...
			if not(self.Seizuring)then
				...
				net.WriteBit(true)
				net.Send(self)
				self:PrintMessage(HUD_PRINTTALK,"你的癫痫正在发作。")
				self:PrintMessage(HUD_PRINTCENTER,"癫痫")
