...
function GM:EndTheRound(reason, murderer)
	...
	if(reason==4)then
		if murderer then
			...
			local msgs
			if(self.SHTF)then
				msgs = Translator:AdvVarTranslate("凶手失去了杀人的意愿，凶手是 {murderer}.\n每个人都幸福地生活在一起。", {
					murderer = {text = murderer:Nick() .. ", " .. murderer:GetBystanderName(), color = Color(col.x * 255, col.y * 255, col.z * 255)}
				})
			else
				msgs = Translator:AdvVarTranslate("叛徒失去了谋杀的意愿，这个叛徒是 {murderer}.\n每个人都幸福地生活在一起。", {
					murderer = {text = murderer:Nick() .. ", " .. murderer:GetBystanderName(), color = Color(col.x * 255, col.y * 255, col.z * 255)}
				})
			end
	...
	elseif reason == 3 then
		if murderer then
			...
			local msgs
			if(self.SHTF)then
				msgs=Translator:AdvVarTranslate("叛徒的愤怒退出了，它是 {murderer}", {
					murderer = {text = murderer:Nick() .. ", " .. murderer:GetBystanderName(), color = Color(col.x * 255, col.y * 255, col.z * 255)}
				})
			else
	...
	elseif reason == 2 then
		...
		if(RealName==GameName)then
			if(self.SHTF)then
				msgs=Translator:AdvVarTranslate("叛徒是 {murderer}",{
	...
	elseif reason == 1 then
		...
		local ct = ChatText()
		if(self.ZOMBIE)then
			ct:Add("僵尸获胜！", Color(190, 20, 20))
		elseif(self.SHTF)then
			ct:Add("叛徒获胜！", Color(190, 20, 20))
		else
	...
	elseif(reason==5)then
		...
		local col = murderer:GetPlayerColor()
		local msgs,RealName,GameName=nil,murderer:Nick(),murderer:GetBystanderName()
		msgs=Translator:AdvVarTranslate("{murderer} 获胜!", {
			murderer = {text = GameName, color = Color(col.x * 255, col.y * 255, col.z * 255)}
		})
		local ct = ChatText()
		ct:AddParts(msgs)
	...
	elseif(reason==6)then
		msgs=Translator:AdvVarTranslate("全员死亡，游戏结束",{})
		local ct = ChatText()
		ct:AddParts(msgs)
		ct:SendAll()
	elseif(reason==7)then
		msgs=Translator:AdvVarTranslate("时间到，游戏结束",{})
		local ct = ChatText()
		ct:AddParts(msgs)
		ct:SendAll()
	end
	...
	print("Homicide 平衡报告： 凶手 "..tostring(self.MurdererWins).." 平民 "..tostring(self.BystanderWins))
		...
		if(math.random(1,8)==3)then
			...
			if(math.Rand(0,1)>Chance)then
				if(self.EnoughAINodes)then
					...
					print("Homicide: 地图上没有足够的AI节点用于僵尸爆发")
				end
			end
		end
		if(ForceZombi)then
...
function GM:RotateMap()
	...
	print("[Homicide] 将换到下一个地图： " .. nextMap)
	local ct = ChatText()
