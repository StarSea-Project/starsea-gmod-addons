...
function GM:RenderSpectate()
	if self:IsCSpectating() then
		...
		if IsValid(self:GetCSpectatee()) && self:GetCSpectatee():IsPlayer() then
			...
			local lp3 = LocalPlayer()
			local obp5 = lp3:GetObserverTarget()
			local wol2,mc,IsMurder,obH
			
			if IsValid(obp5) then
			    wol2 = obp5:Name()
				obH = "生命:"..obp5:Health()
				if obp5.Murderer then
				    mc = obp5:GetPlayerColor() or Color(255,255,255)
					IsMurder = "(杀手)"
				else
				    mc = obp5:GetPlayerColor() or Color(255,255,255)
					IsMurder = ""
			    ...
			    wol2 = "nil"
				mc = Color(255,255,255)
				IsMurder = ""
				obH = ""
			end
			
			mcl = Color(mc.x * 255,mc.y * 255,mc.z * 255)
			drawTextShadow(wol2.." | "..obp5:GetBystanderName()..IsMurder, "MersRadialSemiSuperS", ScrW() / 2, 100, mcl, 1)
			drawTextShadow(obH.."", "MersRadialSemiSuperS", ScrW() / 2, 600, mcl, 1)
