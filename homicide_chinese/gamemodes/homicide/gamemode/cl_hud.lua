...
function GM:DrawStartRoundInformation()
	...
	elseif(self.ZOMBIE)then
		t1="你是生还者！"
		desc={"要么杀死阿尔法僵尸，要么逃跑，要么等待军队的到来。"}
	elseif(self.SHTF)then
		t1="你是无辜的平民"
		desc={"集团中存在一个贪婪、凶残的叛徒","警惕起来！"}
	end
	...
	if LocalPlayer().Murderer then
		...
		if(self.ZOMBIE)then
			t1="你是阿尔法僵尸！"
			desc={"带领你的僵尸军团","吃掉那些人类的大脑！"}
		elseif(self.SHTF)then
			t1="你是叛徒！"
			desc={"杀死所有人，拿走他们的东西","别被抓住了!"}
		end
		c = Color(190, 20, 20)
	end
	...
	if hasMagnum then
		...
		if(self.ZOMBIE)then
			t1="你是生还者"
			t2="你有一把大型武器"
			desc={"杀死阿尔法僵尸，或者","杀死所有其他僵尸，或者","藏起来直到军队到来"}
		elseif(self.SHTF)then
			t1="你是无辜的平民"
			t2="你有一把大型武器"
			desc={"找出并杀死叛徒","保护其他无辜的人"}
		end
	end
	...
	if(self.SHTF)then
		if(self.DEATHMATCH)then
			Txt=Txt.."死亡竞赛，j"
			draw.SimpleText("经典的自由竞争的死亡竞赛。技能奖励增加。", "MersRadialSmall",ScrW()/2,ScrH()*.15,Col2,TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)
		elseif(self.ZOMBIE)then
			Txt=Txt.."僵尸爆发模式"
			draw.SimpleText("一场僵尸感染已经爆发了。活着的人必须与首领僵尸和他的成群结队的僵尸战斗。", "MersRadialSmall",ScrW()/2,ScrH()*.15,Col2,TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)		
		else
			Txt=Txt.."紧急状态模式"
			draw.SimpleText("当地社会在一场灾难后瓦解了，群体中出现了一个贪婪、凶残的叛徒。", "MersRadialSmall",ScrW()/2,ScrH()*.15,Col2,TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)
		end
	else
		if(self.PUSSY)then
			Txt=Txt.."无枪模式"
			draw.SimpleText("在这些地方禁止使用枪支。有一个连环杀手，但警方会很快到达！努力藏起来！等待警察的到来", "MersRadialSmall",ScrW()/2,ScrH()*.15,Col2,TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)
		elseif(self.EPIC)then
			Txt=Txt.."西部牛仔模式"
			draw.SimpleText("有一个杀手在逃。每个人都有枪支，包括凶手。警察们反应迟钝。", "MersRadialSmall",ScrW()/2,ScrH()*.15,Col2,TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)
		elseif(self.ISLAM)then
			Txt=Txt.."圣战模式"
			draw.SimpleText("有一个有自杀倾向的疯子在逃，试图炸死所有人。", "MersRadialSmall",ScrW()/2,ScrH()*.15,Col2,TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)
		else
			Txt=Txt.."经典模式"
			draw.SimpleText("这就像其他任何一天，除了有一个连环杀手在逃。", "MersRadialSmall",ScrW()/2,ScrH()*.15,Col2,TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)
		end
	end
...
function GM:DrawGameHUD(ply)
	...
	if shouldDraw != false then
		...
		if shouldDraw!=false then
			...
			if(self.SHTF)then Name="无辜者" end
			if(self.DEATHMATCH)then Name="斗士" end
			if(self.ZOMBIE)then Name="生还者" end
	...
	if self:GetRound() == 1 then
		...
		if(self.TKerPentalty!=0)then
			if(self.TKerUnShowTime>CurTime())then
				if(self.TKerPenalty==1)then
					...
					if(self.SHTF)then
						drawTextShadow("下一轮你将成为无辜者。", "MersRadial", ScrW() * 0.5, ScrH()/2, Color(90,20,20), 1, TEXT_ALIGN_CENTER)
					else
						drawTextShadow("下一轮你将成为平民。", "MersRadial", ScrW() * 0.5, ScrH()/2, Color(90,20,20), 1, TEXT_ALIGN_CENTER)
					end
					AlreadyDrawn=true
				elseif(self.TKerPenalty==2)then
					surface.SetDrawColor(10,10,10,50)
					drawTextShadow("下一轮你将成为旁观者。", "MersRadial", ScrW() * 0.5, ScrH()/2, Color(90,20,20), 1, TEXT_ALIGN_CENTER)
					AlreadyDrawn=true
				end
			end
		end
		if not(AlreadyDrawn)then
			...
			if((Ply.InnocenceLost)and(Ply.InnocenceLost>CurTime()))then
				surface.SetDrawColor(10,10,10,50)
				drawTextShadow("你已经失去了你的清白。", "MersRadial", ScrW() * .5, ScrH()*.85, Color(90,20,20), 1, TEXT_ALIGN_CENTER)
			end
		end
	end
end
